...
namespace CalamityLegendsReturn.Weapons.A_Dev.PeaShooter
{
    ...
    internal sealed class PeaShooterPea : ModProjectile, ILocalizedModType
    {
        ...
        {
            if (Main.rand.NextBool(2))
                CLCRLightingBoltsSystem.Spawn_PeaShooterBlueStars(Projectile.Center, Projectile.velocity.SafeNormalize(Vector2.UnitX), 0.42f);
        ...
        internal static void SpawnImpactVisuals(Vector2 center, PeaShooterPeaType peaType, float scale)
        {
            ...
            if (peaType == PeaShooterPeaType.Starlight)
                CLCRLightingBoltsSystem.Spawn_PeaShooterBlueStars(center, Vector2.Zero, 1.25f * scale);
